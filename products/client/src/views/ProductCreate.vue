...
<main class="mt-3">
  <div class="container">
      <h2 class="text-center pb-3">제품 등록</h2>
...
export default {
    ...
    methods: {
        ...
        productInsert(){
            if(this.product.product_name == "") return this.$swal("제품명은 필수 입력항목입니다.");
            if(this.product.product_price == 0 || this.product.product_price == "") 
                return this.$swal("제품가격은 필수 입력항목입니다.");
            if(this.product.outbound_days == "" || this.product.outbound_days == 0)
                return this.$swal("출고일은 필수 입력항목입니다.");
